...
def get_player_stats(player_id: int, competition_id: int, season: int):
  ...
  for attempt in range(1, MAX_RETRIES + 1):
    try:
      conn.request("GET", f"/api/v1/competitions/{competition_id}/seasons/{season}/playerinfo/{player_id}")
...
def save_player_info_to_file(player_id: str, competition_id: int, season: int, info_data: str):
  directory_path = Path(f"{PLAYERS_PATH}/competition={competition_id}/season={season}/{player_id}")

  file_path = directory_path / f"info.json"
...
def main():
  ...
  for player_id in player_ids:  # Process player IDs
    ...
    for raw in player_data:
      ...
      ## Check if the info file for the player exists, if not create it
      if file_exists(f"{PLAYERS_PATH}/competition={competition_id}/season={season}/{player_id}/info.json"):
        print(f"Info file already exists for player {player_id}, competition {competition_id}, season {season}, skipping...")
        continue
      ...
      if ret:
        save_player_info_to_file(player_id, competition_id, season, stats)
